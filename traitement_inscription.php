...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($nom) || empty($prenom) || empty($email) || empty($motdepasse)) {
        echo "Erreur : veuillez remplir tous les champs obligatoires. <a href='inscription.php>";
        exit;
    }
    ...
    try {
        ...
        // ✅ INSERT corrigé
        $stmt = $pdo->prepare("
            INSERT INTO anciens_stagiaires 
            (nom, prenom, email, ecole, annee_stage, duree_stage, motdepasse, recontacter)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ");
