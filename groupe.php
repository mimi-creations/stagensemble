...
$stmtCheck = $pdo->prepare("SELECT id FROM groupe_membres WHERE groupe_id = ? AND utilisateur_id = ?");
...
$stmtMembres = $pdo->prepare("
    ...
    JOIN anciens_stagiaires a ON a.id = gm.utilisateur_id
    WHERE gm.groupe_id = ?
...
function formatDate($date_envoi) {
    $date      = new DateTime($date_envoi);
    $today     = new DateTime('today');
    $yesterday = new DateTime('yesterday');
    if ($date >= $today)     return "Aujourd'hui à "    . $date->format('H:i');
    if ($date >= $yesterday) return "Hier à "           . $date->format('H:i');
...
<head>
    ...
    <style>
        .whatsapp-container { display: flex; height: calc(100vh - 62px); }
        .sidebar { width: 300px; min-width: 260px; border-right: 1px solid #eef0ff; display: flex; flex-direction: column; background: #fff; overflow: hidden; }
        .sidebar-header { padding: 16px 18px; font-weight: 700; font-size: 1rem; color: var(--color-primary, #0077b5); border-bottom: 1px solid #eef0ff; }
        .chat-area { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
        .contact-list { overflow-y: auto; flex: 1; }
        .contact-item { display: flex; align-items: center; gap: 10px; padding: 10px 15px; text-decoration: none; color: inherit; border-bottom: 1px solid #f3f4f6; transition: background 0.12s; }
        .contact-item:hover { background: #f8fafc; }
        .contact-item.active { background: #e0f2fe; }
        .membres-chips { display: flex; flex-wrap: wrap; gap: 6px; padding: 10px 18px; border-bottom: 1px solid #eef0ff; background: #f8fafc; }
        .chip { display: flex; align-items: center; gap: 5px; background: #e0f2fe; border-radius: 20px; padding: 3px 10px 3px 5px; font-size: 0.78rem; color: #0369a1; }
        .chip img { width: 20px; height: 20px; border-radius: 50%; object-fit: cover; }
        .chat-header { padding: 12px 20px; background: #fff; border-bottom: 1px solid #eef0ff; display: flex; align-items: center; gap: 14px; }
        .chat-header img { width: 42px; height: 42px; border-radius: 50%; object-fit: cover; border: 2px solid #e0f2fe; }
        .chat-header-emoji { width: 42px; height: 42px; border-radius: 50%; background: #e0f2fe; display: flex; align-items: center; justify-content: center; font-size: 1.3rem; }
        .chat-header-info { flex: 1; }
        .chat-header-info h2 { margin: 0; font-size: 1rem; color: var(--color-primary, #0077b5); }
        .chat-header-info span { font-size: 0.78rem; color: #6b7280; }
        .chat-box { flex: 1; padding: 20px; overflow-y: auto; background: #f8fafc; display: flex; flex-direction: column; gap: 12px; }
        .bubble { position: relative; padding: 10px 15px 28px; border-radius: 14px; max-width: 62%; width: fit-content; }
        ...
        .bubble .bubble-time { position: absolute; bottom: 5px; right: 10px; font-size: 0.67rem; color: rgba(0,0,0,0.38); white-space: nowrap; }
        .bubble .bubble-avatar { width: 26px; height: 26px; border-radius: 50%; object-fit: cover; position: absolute; bottom: -5px; left: -30px; }
        .chat-form { padding: 14px 20px; background: #fff; border-top: 1px solid #eef0ff; display: flex; gap: 10px; }
        .chat-form input[type="text"] { flex: 1; padding: 11px 16px; border: 1px solid #e2e8f0; border-radius: 25px; outline: none; font-size: 0.92rem; }
        .chat-form input[type="text"]:focus { border-color: #0077b5; }
        .chat-form button { padding: 11px 24px; background: var(--color-accent, #c8102e); color: #fff; border: none; border-radius: 25px; cursor: pointer; font-weight: bold; font-size: 0.9rem; }
        .chat-form button:hover { opacity: 0.85; }
        .btn-retour { display: inline-flex; align-items: center; gap: 5px; padding: 7px 14px; background: #f1f5f9; color: #374151; border: none; border-radius: 8px; text-decoration: none; font-size: 0.83rem; cursor: pointer; }
        .btn-retour:hover { background: #e2e8f0; }
        .groupe-badge { font-size: 0.7rem; background: #e0f2fe; color: #0369a1; padding: 2px 7px; border-radius: 10px; margin-left: 5px; }
        .modal-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.45); z-index: 1000; align-items: center; justify-content: center; }
        .modal-overlay.active { display: flex; }
        .modal-box { background: #fff; border-radius: 16px; padding: 28px 32px; width: 460px; max-width: 95vw; max-height: 85vh; overflow-y: auto; box-shadow: 0 8px 40px rgba(0,0,0,0.18); }
        ...
        .modal-box input[type="text"], .modal-box input[type="file"] { width: 100%; padding: 9px 12px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 0.9rem; margin-bottom: 16px; box-sizing: border-box; }
        .membres-list { display: flex; flex-direction: column; gap: 8px; max-height: 200px; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 10px; margin-bottom: 20px; }
        .membre-item { display: flex; align-items: center; gap: 10px; padding: 6px 8px; border-radius: 8px; cursor: pointer; }
        .membre-item:hover { background: #f3f4f6; }
        .membre-item input[type="checkbox"] { accent-color: #0077b5; width: 16px; height: 16px; }
        ...
        #preview-photo { width: 60px; height: 60px; border-radius: 50%; object-fit: cover; display: none; margin-bottom: 12px; }
        ...
        .sidebar-tab.active { color: #0077b5; border-bottom-color: #0077b5; }
        .tab-panel { display: none; }
        .tab-panel.active { display: block; }
        .btn-nouveau-groupe { display: flex; align-items: center; gap: 6px; margin: 10px 15px; padding: 8px 14px; background: var(--color-accent, #c8102e); color: #fff; border: none; border-radius: 20px; cursor: pointer; font-size: 0.85rem; font-weight: 600; }
...
<body>
    ...
    <div class="whatsapp-container">
        ...
        <div class="sidebar">
            ...
            <div class="tab-panel active" id="panel-groupes">
                ...
                <div class="contact-list">
                    <?php if (empty($mesGroupes)): ?>
                        <p style="text-align:center; color:#aaa; padding:20px; font-size:0.88rem; font-style:italic;">Aucun groupe.</p>
                    <?php else: ?>
                        <?php foreach ($mesGroupes as $g): ?>
                            <a href="groupe.php?id=<?= $g['id'] ?>" class="contact-item <?= $g['id'] == $groupe_id ? 'active' : '' ?>">
                                <div style="width:40px; height:40px; flex-shrink:0;">
                                    <?php if (!empty($g['image'])): ?>
                                        <img src="<?= htmlspecialchars($g['image']) ?>" style="width:40px;height:40px;border-radius:50%;object-fit:cover;">
                                    <?php else: ?>
                                ...
                                <div style="flex:1; min-width:0;">
                                    <h3 style="margin:0; font-size:0.92rem;"><?= htmlspecialchars($g['nom']) ?> <span class="groupe-badge">Groupe</span></h3>
            ...
            <div class="tab-panel" id="panel-prives"></div>
        </div>
        ...
        <div class="chat-area">
            ...
            <div class="membres-chips">
                <?php foreach ($membres as $m): ?>
                    <div class="chip">
                        <img src="<?= htmlspecialchars($m['avatar'] ?? 'default_avatar.png') ?>" alt="<?= htmlspecialchars($m['prenom']) ?>">
            ...
            <div class="chat-box" id="chatBox">
                <?php if (empty($messages)): ?>
                    <p style="text-align:center; color:#aaa; margin-top:30px; font-style:italic;">Soyez le premier à écrire dans ce groupe !</p>
                <?php else: ?>
                    <?php foreach ($messages as $msg): ?>
                        <?php $estMoi = ($msg['expediteur_id'] == $mon_id); ?>
                        <div class="bubble <?= $estMoi ? 'me' : 'other' ?>" style="<?= !$estMoi ? 'margin-left:36px;' : '' ?>">
                            <?php if (!$estMoi): ?>
                            ...
                            <span class="bubble-time"><?= formatDate($msg['date_envoi']) ?></span>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
                ...
                <div class="membres-list">
                    <?php foreach ($tousStagiaires as $s): ?>
                        <label class="membre-item">
                            ...
                            <img src="<?= htmlspecialchars($s['avatar'] ?? 'default_avatar.png') ?>" alt="<?= htmlspecialchars($s['prenom']) ?>">
